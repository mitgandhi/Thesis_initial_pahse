...
def bushing_pressure_deformation(filepath, m, lF, n, deltap, plots, degstep, ognore, minplot, maxplot, offset):
    ignore = ognore + offset
    data = load_matlab_txt(os.path.join(filepath, 'output', 'piston', 'matlab', 'Piston_Cylinder_Gap_Pressure_Deformation.txt'))
    ...
    out_dir = os.path.join(filepath, 'output', 'piston', 'Plots', 'Bushing_Pressure_Deformation')
    ...
    print(f"\nGenerating bushing pressure deformation plots for: {filepath}")
    progress = 0
    ...
    for i in range(plots):
        ...
        ax.set_title(f'Bushing Pressure Deformation\nn={n}, ΔP={deltap}, φ={deg:.1f}°')
    ...
    savemat(os.path.join(out_dir, 'bushing_deformation_data.mat'), {'comparisonData': summary})

    plt.figure()
    plt.plot(degrees, summary['minDeformation'], 'b-', linewidth=1.5)
    plt.title('Min Bushing Deformation vs. Shaft Angle')
    plt.xlabel('Shaft Angle [°]')
    plt.ylabel('Min Bushing Deformation [μm]')
    plt.grid(True)
    plt.savefig(os.path.join(out_dir, 'Min_Bushing_Deformation_vs_Shaft_Angle.png'))
    plt.close()

    plt.figure()
    plt.plot(degrees, summary['maxDeformation'], 'r-', linewidth=1.5)
    plt.title('Max Bushing Deformation vs. Shaft Angle')
    plt.xlabel('Shaft Angle [°]')
    plt.ylabel('Max Bushing Deformation [μm]')
    plt.grid(True)
    plt.savefig(os.path.join(out_dir, 'Max_Bushing_Deformation_vs_Shaft_Angle.png'))
    plt.close()

    print(f"\nMax Bushing Deformation: {globalMaxVal:.2f} µm @ {phiMax:.1f}°")
    print(f"Min Bushing Deformation: {globalMinVal:.2f} µm @ {phiMin:.1f}°")
